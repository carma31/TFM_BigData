...
createDirectoryIfNotExists(absoluteClusteringFullStatisticsDir)

# Delete previus executions data
deleteDirectoryData(absoluteClusteringFullStatisticsDir)

# Get all models evalutions
mEvaluations = sorted(getFilesInDir(absoluteClusteringFullClassResDir))
...
for modelEval in mEvaluations:
	...
	rawData = sc.textFile(absoluteClusteringFullClassResDir + "/" + modelEval)
	...
	# Write result data
	writeCSV(absoluteClusteringFullStatisticsDir + '/' + clustersPerCustomerClustFileName + '-100-' + modelEval, res)
	...
	# Write result data
	writeCSV(absoluteClusteringFullStatisticsDir + '/' + perCustomerClustFileName + '-100-' + modelEval, res)
	...
	for i in xrange(5, 100, 5):
		...
		# Write result data
		writeCSV(absoluteClusteringFullStatisticsDir + '/' + perCustomerClustFileName + '-' + str(i).zfill(3) +'-' + modelEval, res)
		...
		# Write result data
		writeCSV(absoluteClusteringFullStatisticsDir + '/' + clustersPerCustomerClustFileName + '-' + str(i).zfill(3) + '-' + modelEval, res)
		...
		# Wirte result data
		writeCSV(absoluteClusteringFullStatisticsDir + '/' + clustersStatisticsFileName + '-' + str(i).zfill(3) + '-' + modelEval, res)
	...
	# Wirte result data
	writeCSV(absoluteClusteringFullStatisticsDir + '/' + clustersStatisticsFileName + '-100-' + modelEval, res)
