...
createDirectoryIfNotExists(absoluteClusteringFullClassResDir)

# Delete previus executions data
deleteDirectoryData(absoluteClusteringFullClassResDir)

# Get all generated models
models = sorted(getFilesInDir(absoluteClusteringFullModelsDirName))
models = filter(lambda x: "0001" not in x and ".txt" in x, models)
...
for model in models:
	...
	gmm.load_from_text(absoluteClusteringFullModelsDirName + "/" + model)

	# Get sample components
	n_components = gmm.n_components

	dataset = open(absoluteFullClusterDataset, "r")
	result = open(absoluteClusteringFullClassResDir + "/" + str(n_components).zfill(4) + ".csv", "w")
