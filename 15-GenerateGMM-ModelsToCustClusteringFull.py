...
deleteDirectoryData(absoluteReclusteringFullDir)

# Get Datasets to reclustering process
normalizedDatasets = sorted(getFilesInDir(absoluteClusteringFullStatisticsDir))
normalizedDatasets = filter(lambda x: "perCustomer-" in x and "-0001" not in x, normalizedDatasets)
...
for f in normalizedDatasets:
	...
	rawData = spark_context.textFile(absoluteClusteringFullStatisticsDir + "/" + f)
	# Remove brakets
	...
	# Write CSV result
	writeCSV(absoluteReclusteringFullDir + '/' + auxFileName, res)
...
for trainingFile in trainingFiles:
	...
	# Load dataset
	text_lines = spark_context.textFile(absoluteReclusteringFullDir + "/" + trainingFile)
	...
	for cType in reclust_covar_types:
		...
		auxLogDirName = absoluteReclusteringFullLogDirName + "-" + trainingFile + "-" + cType[1]
		auxModelsDirName = absoluteReclusteringFullModelsDirName + "-" + trainingFile + "-" + cType[1]
